...
export function getTransformerForTool(
  ...
  if (invocation !== undefined) {
    // Always normalize command references at the target boundary. This is
    // intentionally not gated on whether the target matches the canonical
    // style: legacy/custom `/opsx:<id>` input must still become `/opsx-<id>`
    // for flat tools instead of leaking through unchanged.
